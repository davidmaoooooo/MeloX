    ...
    var body: some View {
        ...
        Form {
            ...
            Section {
                ...
                Text("当前句及之前的歌词先移动，等待“后续歌词启动延迟”（默认 40 毫秒）后，下一句及之后的歌词才开始位移；后续歌词的实际延迟再由“基础拖尾延迟”和“逐句拖尾增量”完整累计，不设内部最大延迟；三项均可调至 0 毫秒。“拖尾追赶节奏”默认 95%，数值越低追赶越快。“追赶速度梯度”和“回弹强度梯度”在 0% 时各句一致，100% 时完整逐句增强，默认分别为 75% 和 85%。位移基准时长为 0.68 秒，默认开启 26% 位移回弹；当前句升格放大另有独立的 0.58 秒、32% 回弹。瞬移阈值默认 260 毫秒，可在 50–500 毫秒之间调整；剩余时间短于该阈值时会直接对齐。")
            }
